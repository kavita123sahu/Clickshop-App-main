...
const SubSubCategory = (props: any) => {
    const { SubSubCategoryData, title } = props.route.params;
    const { navigation } = props;
    const [searchQuery, setSearchQuery] = useState('');
    const [filteredBooks, setFilteredBooks] = useState(SubSubCategoryData);
    const [subCatData, setSubCatData] = useState(SubSubCategoryData);

    const onClickItems = async (item: any) => {
        try {
            const result: any = await _CAT_SERVICEs.get_sub3_categories(item.id);
            if (result?.sub3Category.length > 0) {
                props.navigation.navigate('Sub3Category', { Sub3CategoryData: result?.sub3Category, title: item?.name });
            } else {
                navigation.navigate('AllBooks', { link: `https://clikshop.co.in/api/v3/products?sub_sub_category=${item.id}` })
            }
        } catch (error) {
